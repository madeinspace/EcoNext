...
const q = ({ ClientID, BMID, sStartYear, sEndYear, WebID, imptype }) => {
  let exportFunc = ``;
  switch (imptype) {
    case '1':
  ...
  return {
    query: `select * from CommData_Economy.[dbo].[${exportFunc}](?,?,?,?,?,?,? )`,
    params: [ClientID, WebID, BMID, sStartYear, sEndYear, 1, null],
  };
};
/* #endregion */
