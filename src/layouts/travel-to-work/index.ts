...
const contentDataQuery = ({ ClientID, IGBMID, Sex, Indkey, WebID }) =>
  `select * from CommData_Economy.[dbo].[fn_Industry_TravelMethod_Sex]( ${ClientID}, ${WebID}, ${IGBMID}, 2016, 2011, 'UR', 3, 1, null, ${Indkey} ) order by LabelKey ASC`;
...
const Subtitle = ({ toggles }) => {
  return `Resident workers - Method of travel to work - ${toggles.currentIndustryName}`;
};
...
const Headline = ({ toggles, data }) => {
  ...
  const headlineAlt = `Within ${prefixedAreaName}, there is a ${comparison} proportion of resident ${currentIndustryName} workers using public transport to get to work than the ${currentBenchmarkName} workforce.`;
