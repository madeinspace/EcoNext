...
const DestByOccupationQuery = ({ ClientID, Indkey }) => {
  const query = `select * from CommData_Economy.[dbo].[fn_AD_JTW_Dest_LGA_by_Industry_Summ_2016](${ClientID},10,NULL,23000) order by LabelKey`;
  return query;
};

const DestByOccupationAllQuery = ({ ClientID, Indkey }) => {
  const query = `select * from CommData_Economy.[dbo].[fn_AD_JTW_Dest_LGA_by_Industry_2016](${ClientID},10,NULL,23000) Where Number > 9 order by Number DESC`;
  return query;
};
const fetchData = async ({ filters }) => {
  ...
  const layersUrl = `https://economy.id.com.au/${clientAlias}/geo/areasbyentityid/6574?ClientID=${ClientID}&WebID=10&LGACode=0&Indkey=23000`;
  const thematicUrl = `https://economy.id.com.au/${clientAlias}/geo/data?ClientID=${ClientID}&WebID=10&LGACode=0&Indkey=23000&dataid=37`;
  ...
  entities: [
    {
      ...
      renderString: ({ data }): string => {
        return `Workers place of residence`;
      },
    },
    {
      Title: 'DataSource',
