...
export function SearchResultsList(props: ISearchResultsListProps) {
  let ResultsList: any;
  const { results, noResults, searchOption, hasRegistered, clientName } = props;

  const handleResultClick = (e: React.MouseEvent<HTMLElement>) => {
    e.preventDefault();
    const redirect = encodeURIComponent(window.location.href);
    window.open(`https://content.id.com.au/community-access?redirect=${redirect}`, '_self');
  };
  ...
  if (searchOption === 'place' && !noResults) {
    const placesInArea = results
      ...
      .map((item, i) => {
        return <SearchResultPlace key={'inarea' + i.toString()} itemData={item} inArea={item.inArea} />;
      });

    const placesOutsideArea = results
      .filter(item => !item.inArea)
      .map((searchitem, j) => {
        const item: any = <SearchResultPlace key={j} itemData={searchitem} hasRegistered={hasRegistered} />;
    ...
    ResultsList = (
      <div>
        ...
        {placesInArea.length > 0 ? (
          ...
          <div className={'placeInsideList'}>No places matching your search were found in {clientName}</div>
        )}
        {placesOutsideArea.length > 0 ? (
          <div>
            <h3 className={'placeOutsideTitle'}>
              PLACES OUTSIDE {clientName}{' '}
              {!hasRegistered ? <span onClick={handleResultClick}>(CLICK RESULTS TO UNLOCK ALL PLACES)</span> : null}
            </h3>
